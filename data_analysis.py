...
# DATA_FOLDERS = ["barb", "joer", "kpop", "nfts", "star"]
# DATA_FOLDERS = ["dsmp"]
...
with open("data/1000common.txt") as f:
    ...
    CUSTOM_FILTER_WORDS.remove("dream")
...
for data_folder_name in DATA_FOLDERS:
    ...
    tweets = json.loads(
        open("data/" + data_folder_name + "/tweet_text.json", "r").read()
    )
    ...
    plt.savefig(
        "analysis_result/" + data_folder_name + "/sentiment_analysis_result.png"
    )
    plt.close(fig)
    ...
    with open(
        "analysis_result/" + data_folder_name + "/word_frequency.json", "w"
    ) as outfile:
...
for index in range(len(DATA_FOLDERS)):
    ...
    fig.savefig("analysis_result/" + DATA_FOLDERS[index] + "/frequency_analysis.png")
